...
def map_transaction_state_data_conversion(cursor):
    ...
    for data in map_transaction_data:
        for hover_data in data.data.hoverDataList:
            district_id = get_district_id(cursor, hover_data.name)
            ...
            query = """
                ...
                (district_id, count, amount, year_with_quarter) 
                VALUES (%s, %s, %s, %s);
                """

            cursor.execute(query, (district_id, count, amount_str, year_with_quarter))
